...
def get_arguments(args=None):
    ...
    # TRAINING
    parser.add_argument('--num_epoch', help='Number of epochs for training', type=int, default=150) #250
    ...
    parser.add_argument('--start_eval', help='Epoch at which evaluation will start', type=int, default=0) # Don't start evaluating bf all layers are unfrozen!
    ...
    parser.add_argument('--random_batch_sample', help='Randomly sample 1-2 frames in every n frames', type=float, default=1) 
    parser.add_argument('--sample_depth', type=int, nargs='*', help='Random sample depth', default=[0])

    # prompt
    parser.add_argument('--mpn_use_prompt_edge', type=_store_bool, nargs='*', default=[0]*4)#9
    parser.add_argument('--regenerate_embedding', action='store_true', default=False)
    parser.add_argument('--KL_loss', type=float, default=2)
    parser.add_argument('--use_instance_prompt', action='store_true', default=False)
    
    parser.add_argument('--inst_KL_loss', type=float, default=1)
    ...
    parser.add_argument('--frames_per_level', type=int, nargs='*', help='Number of frames to process at each hierarchical level', default=[5,25,75,150])#, 256, 512
    parser.add_argument('--pruning_method', type=str, nargs='*', help='Pruning scheme used at each hierarchical level', default=['geometry', 'motion_005', 'motion_005', 'motion_005'])

    parser.set_defaults(symmetric_edges=True)  # Graphs contain each edge twice with changing source and destination

    # HIERARCHICAL PARAMETERS
    parser.add_argument('--hicl_depth', help='The depth of the hierarchical architecture', type=int, default=4)#9
    ...
    # MOTION SETTING
    parser.add_argument('--motion_max_length', type=int, nargs='*', help='Maximum number of frames used to encode trajectories before pred at each layer', default=[5,25,75])#, 128, 256
    parser.add_argument('--motion_pred_length', type=int, nargs='*', help='Number of future/past frames for which locations are predicted at each level', default=[5,25,75])#, 128, 256
    ...
    # MOTION FEATURES
    parser.add_argument('--mpn_use_motion', type=_store_bool, nargs='*', help='Use motion predictions GIoU as edge feature at each layer', default=[False, True, True, True])#, True, True
    parser.add_argument('--mpn_use_reid_edge', type=_store_bool, nargs='*', default=[True]*4)#9
    parser.add_argument('--mpn_use_pos_edge', type=_store_bool, nargs='*', default=[True]*4)#9
...
def create_experiment_path(config):
    ...
    if not os.path.exists(config.experiment_path):
        ...
        print(f"Create exp path:{config.experiment_path}......")
    else:
        print(f"Exp path exists!")
