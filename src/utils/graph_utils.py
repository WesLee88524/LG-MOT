...
def compute_edge_features(edge_ixs, node_frames, node_bboxes, node_feets, node_reids, fps, motion_feats={}, prompt_edge=None, x_conf=None,
                          edge_feats_to_use=(
                                             'secs_time_dists', 'norm_feet_x_dists', 'norm_feet_y_dists',
                                             'bb_height_dists', 'bb_width_dists', 'emb_dists','prompt_edge'),
                                             reid_sim_fn=F.pairwise_distance):
    """
    ...
    if edge_ixs.numel():
        ...
        if prompt_edge != None and 'train' == prompt_edge["mode"]:
            # print(f"It's training....")
            edge_feats_dict['prompt_edge'] = load_prompt_edge_from_pt(prompt_edge,len(col)).to(edge_feats_dict['vel_row_x'].device).T
        elif prompt_edge != None and ('val' == prompt_edge["mode"] or 'test' in prompt_edge["seq_path"]):
            # print(f"It's validation....")
            tmp = torch.ones(1, 128) # TODO not elegent. Just to take up space
            edge_feats_dict['prompt_edge'] = tmp.expand(len(col), -1).to(edge_feats_dict['vel_row_x'].device).T
        else:
            pass
        ...
        edge_feats = [edge_feats_dict[feat_names] for feat_names in edge_feats_to_use if feat_names in edge_feats_dict and feat_names!='prompt_edge']
        if prompt_edge != None :
            t1 = torch.stack(edge_feats).T
            t2 = edge_feats_dict['prompt_edge'].T
            edge_feats = torch.cat((t1, t2), dim=1)
        else:
...
def load_prompt_edge_from_pt(prompt_edge,n):
    prompt_embeddings_path = prompt_edge["seq_path"]+'/processed_data/embeddings/'+prompt_edge["seq"]+'_prompt_embedding_all.pt'
    prompt_feature =  torch.load(prompt_embeddings_path).to(torch.float32)
    return prompt_feature.expand(n, -1)
