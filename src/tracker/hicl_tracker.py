...
import torch
from src.utils.graph_utils import to_undirected_graph, to_lightweight_graph, to_positive_decision_graph, exchange_reid_in_same_block, exchange_node_in_diff_blocks
...
class HICLTracker:
    ...
    def _get_model(self):
        ...
        mpntrack_params['graph_model_params']['encoder_feats_dict']['node_in_dim'] = self.config.node_dim # 2048
        ...
        return HICLNet(submodel_type=MOTMPNet, submodel_params=mpntrack_params['graph_model_params'],
                       ...
                       use_prompt_edge=self.config.mpn_use_prompt_edge,
    ...
    def _postprocess_graph(self, graph, remove_negatives=True, decision_threshold=0.5):
        ...
        """
        to_undirected_graph(graph, attrs_to_update=('edge_preds', 'edge_labels')) # make every edge compute once
        ...
        for step in range(num_steps):
            if self.config.no_fp_loss and torch.any(edge_mask).item():
                ...
                if self.config.KL_loss:
                    loss += self.config.KL_loss*outputs['kl_divergence'][step].view(-1)[edge_mask]
            else:
                loss += self.loss_function(outputs['classified_edges'][step].view(-1), edge_labels.view(-1))
                if self.config.KL_loss:
                    loss += self.config.KL_loss*outputs['kl_divergence'][step].view(-1)
        if self.config.inst_KL_loss:
            loss += self.config.inst_KL_loss*outputs['inst_KL'][0]
        return loss
    ...
    def hicl_forward(self, hicl_graphs, logs, oracle, mode, max_depth, project_max_depth):
        ...
        for curr_depth in range(max_depth):
            ...
            if curr_batch.edge_index.numel(): # number of edge_index               
                if oracle:
                    # Oracle results
                    curr_batch.edge_preds = curr_batch.edge_labels
                    logs[curr_depth]["Loss"].append(0.)
                    # Calculate batch classification metrics and loss # Calculate TP, FP, TN, FN
    ...
    def train(self):
        ...
        for epoch in range(1, self.config.num_epoch+1):
            ...
            # wandb.log({'Train_Loss': statistics.mean(epoch_train_logs["Loss"])})
        ...
        with torch.no_grad():
            ...
            for seq, seq_and_frames in dataset.sparse_frames_per_seq.items():
                ...
                for seq_name, start_frame, end_frame in seq_and_frames:
                    ...
                    data = dataset.get_graph_from_seq_and_frames(seq_name=seq_name, start_frame=start_frame, end_frame=end_frame,valid_frames=None)
