...
# from src.utils.prompt_utils import get_seqs_prompt_attrs_embedding
...
class MOTSceneDataset:
    ...
    def __init__(self, config, seqs, mode):
        ...
        # Load all dataframes frome seq like "MOT17-02-SDP"
        self.seq_det_dfs, self.seq_info_dicts, self.seq_names = self._load_seq_dfs()

        # Index dataset
        self.seq_and_frames = self._index_dataset()#split data to groups 
    ...
    def _load_seq_dfs(self):
        ...
        for dataset_path, seq_list in self.seqs.items():
            for seq_name in seq_list:
                ...
                seq_det_df = seq_processor.load_or_process_detections()# preprocess detection
    ...
    def _index_dataset(self):
        ...
        for scene in self.seq_names:
            ...
            frames_per_graph = self.config.frames_per_graph #frames in graph，150（4 level）/512 （9 level）
            ...
            for f in frames:
                if not start_frames or f >= start_frames[-1] + self.config.train_dataset_frame_overlap:
                    ...
                    valid_frames = np.arange(f, f + frames_per_graph*self.config.random_batch_sample)
                    valid_frames = random.sample(list(valid_frames),frames_per_graph)
                    valid_frames.sort()
                    ...
                    if (graph_df.frame.min() not in start_frames) and (graph_df.frame.max() not in end_frames) and (
                        ...
                        seq_and_frames.append((scene, graph_df.frame.min(), graph_df.frame.max(),valid_frames))# valid_frames: Storage the truth raletive frames
    ...
    def get_df_from_seq_and_frames(self, seq_name, start_frame, end_frame,index_valid_frames=None):
        """
        ...
        seq_det_df = self.seq_det_dfs[seq_name]  # Sequence specific dets (The whole sequence)
        seq_info_dict = self.seq_info_dicts[seq_name]  # Sequence info dict
        if index_valid_frames != None:
            valid_frames = index_valid_frames
        else:
    ...
    def get_graph_from_seq_and_frames(self, seq_name, start_frame, end_frame,valid_frames=None):
        """
        Main dataloading function. Returns a hierarchical graph belonging to the specified sequence range
        """
        #TODO
        # if self.mode=='train':
        #     graph_df['frame'] = valid_frames

        graph_df, seq_info_dict = self.get_df_from_seq_and_frames(seq_name=seq_name, start_frame=start_frame, end_frame=end_frame,index_valid_frames=valid_frames)
        seq_info_dict['mode'] = self.mode
        ...
        y_inst = torch.zeros(y_id.size(0), 128)

        if self.config.use_instance_prompt and self.mode == 'train':
            # load prompt
            instance_prompt_embeddings = osp.join(seq_info_dict["seq_path"],'gt/instance_language_prompts/seq_inst_prompts_embeddings_128.pt')
            
            seq_inst_embeddings = torch.load(instance_prompt_embeddings)

            # extract index from the first col of y_id 
            id = copy.deepcopy(y_id[:, 1])
            # replace -1 with 0
            mask = (id == -1)
            if 'MOT' in seq_info_dict["seq_path"]:
                id[mask] = 0
            elif 'DANCETRACK' in seq_info_dict["seq_path"]:
                id[mask] = len(seq_inst_embeddings)-1

            if max(id) > len(seq_inst_embeddings) - 1:
                print(seq_info_dict["seq_path"])
            # extract the value from list b by index
            for i in range(len(id)):
                y_inst[i,:] = seq_inst_embeddings[id[i]]
        ...
        # confidence of det
        np_conf = graph_df['conf'].to_numpy()
        tensor_conf = torch.tensor(np_conf)
        conf = tensor_conf.reshape(len(tensor_conf),1)
        ...
        hierarchical_graph = HierarchicalGraph(x_reid=x_reid.float(), x_node=x_node.float(), x_frame=x_frame.long(),
                                               ...
                                               conf=conf.float(),
                                               start_frame=start_frame.long(), end_frame=end_frame.long(),seq_info_dict=seq_info_dict,
                                               y_inst=y_inst)
    ...
    def __getitem__(self, ix):
        seq_name, start_frame, end_frame, valid_frames = self.seq_and_frames[ix]
        return self.get_graph_from_seq_and_frames(seq_name=seq_name, start_frame=start_frame, end_frame=end_frame,valid_frames=valid_frames)