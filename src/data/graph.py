...
class HierarchicalGraph(Data):
    ...
    def _one_hot_encode_frames(self):
        ...
        return F.one_hot(norm_frames, num_classes=self.frames_per_level[-1]*math.ceil((self.end_frame-self.start_frame)/self.frames_per_level[-1]))
    ...
    def _get_curr_graph_specs(self, config):
        """
         ...
         x_conf: confidence of detection
        """
        ...
        if self.curr_depth == 0:
            ...
            x_node = scatter_mean(self.x_node, self.map_from_init, dim=0) # scatter_mean:Averages all values from the self.x_node into out at the indices specified in the self.map_from_init
            ...
            # x_conf = scatter_mean(self.x_conf, self.map_from_init, dim=0)
            x_conf = 1

        return x_node, x_reid, x_frame, x_frame_mask, x_bbox, x_feet, x_center, y_id, x_conf
    ...
    def exchange_node_reid_features(self):
        
        pass

    def construct_curr_graph_nodes(self, config):
        # Get current level features and labels
        x_node, x_reid, x_frame, x_frame_mask, x_bbox, x_feet, x_center, y_id, x_conf = self._get_curr_graph_specs(config)
    ...
    def add_edges_to_curr_graph(self, config, curr_graph):
        ...
        x_conf = curr_graph.x_conf
        x_conf = x_conf.to(device=x_reid.device)
        ...
        if config.mpn_use_prompt_edge[self.curr_depth]:
            edge_feats_to_use_ += ['prompt_edge']
            prompt_edge = curr_graph.seq_info_dict
        else:
            prompt_edge = None
        ...
        curr_graph = Graph(x=curr_graph.x, edge_attr=edge_features, edge_index=edge_ixs, edge_labels=edge_labels,
                           ...
                           edge_preds=edge_preds, x_reid=x_reid, edge_mask=edge_mask,y_inst=curr_graph.y_inst)
