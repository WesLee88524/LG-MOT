...
class MLPGraphIndependent(nn.Module):
    ...
    def __init__(self, edge_in_dim = None, node_in_dim = None, edge_out_dim = None, node_out_dim = None,
        ...
        if node_in_dim is not None : #TODO not elegant
            if node_fc_dims[0] == 512 or node_fc_dims[0] == 128 :
                self.node_mlp1 = MLP(input_dim=node_in_dim, fc_dims=list([node_fc_dims[0],]),
                                    dropout_p=dropout_p, use_batchnorm=use_batchnorm)
                self.node_mlp2 = MLP(input_dim=node_fc_dims[0], fc_dims=list(node_fc_dims[1:]) + [node_out_dim],
                                     dropout_p=dropout_p, use_batchnorm=use_batchnorm)
    ...
    def forward(self, edge_feats = None, nodes_feats = None,extract_fc_layer = None):
        if self.node_mlp is not None:
            if extract_fc_layer == None:
                out_node_feats = self.node_mlp(nodes_feats)
                mid_node_feats =  out_node_feats
            else:
                mid_node_feats = self.node_mlp1(nodes_feats)
                out_node_feats = self.node_mlp2(mid_node_feats)
            
        else:
            out_node_feats = nodes_feats
            mid_node_feats = nodes_feats
        ...
        return out_edge_feats, out_node_feats, mid_node_feats
...
class MOTMPNet(nn.Module):
    ...
    """
    def __init__(self, model_params, bb_encoder = None, motion=None, pos_feats=None, reid=None, prompt_edge=None):
        """
        ...
        #TODO not elegant
        if prompt_edge:
            self.clip_to_avg_pool_to_KL = 1
            self.intance_KL = 1
        else:
            self.clip_to_avg_pool_to_KL = 0
            self.intance_KL = 0
        ...
        self.edge_attr_num = encoder_feats_dict['edge_in_dim']

        if self.clip_to_avg_pool_to_KL:
            encoder_feats_dict['edge_in_dim'] -= 1
        # print("EXPECTED EDGE INPUT DIM??", encoder_feats_dict['edge_in_dim'])

        classifier_feats_dict = model_params['classifier_feats_dict']

        self.encoder = MLPGraphIndependent(**encoder_feats_dict) # initial the architecture of the net
    ...
    def _build_core_MPNet(self, model_params, encoder_feats_dict):
        ...
        edge_factor = 2 if self.reattach_initial_edges else 1 # reattach_initial_edges：True
        node_factor = 2 if self.reattach_initial_nodes else 1 # reattach_initial_nodes：False
    ...
    def forward(self, data, edge_level_embed=None, node_level_embed=None):
        ...
        if len(edge_attr[0])>10: # TODO not elegent

            if self.clip_to_avg_pool_to_KL:
                # AvgPooling the result to (batch_size, 16)
                output_size = (16,)
                avg_pooling_layer = nn.AdaptiveAvgPool1d(output_size)
                avg_pooling_language_prompt = avg_pooling_layer(edge_attr[:,self.edge_attr_num-1:])
                edge_attr = edge_attr[:,:self.edge_attr_num-1]
            
        if self.intance_KL:
            latent_edge_feats, latent_node_feats,mid_node_feats = self.encoder(edge_attr, x, 1) #take the middle layer output to do instance-level ditillation
        else:
            latent_edge_feats, latent_node_feats, mid_node_feats = self.encoder(edge_attr, x)
        ...
        outputs_dict = {'classified_edges': [],
                        ...
                        'inst_KL':[]}
        if self.intance_KL:
            if mid_node_feats.size() == data.y_inst.size():
                inst_KL = torch.nn.functional.kl_div(torch.log_softmax(mid_node_feats, dim=1), torch.softmax(data.y_inst.to(mid_node_feats.device), dim=1), reduction='batchmean')
                outputs_dict['inst_KL'].append(inst_KL)
            else:
                outputs_dict['inst_KL'].append(0)
            pass
        for step in range(1, self.num_enc_steps + 1):
            ...
            if self.reattach_initial_nodes:# Determines whether initially encoded node feats are used during node updates,defaut False
            ...
            if step >= first_class_step:
                latent_edge_feats_temp = latent_edge_feats
                # Classification Step
                dec_edge_feats, _, _ = self.classifier(latent_edge_feats_temp)# latent_edge_feats
                outputs_dict['classified_edges'].append(dec_edge_feats)
                if self.clip_to_avg_pool_to_KL:
                    kl_divergence = torch.nn.functional.kl_div(torch.log_softmax(latent_edge_feats_temp, dim=1), torch.softmax(avg_pooling_language_prompt, dim=1), reduction='batchmean')
                    outputs_dict['kl_divergence'].append(kl_divergence)
                else:
                    pass
